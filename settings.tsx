...
function InstallBlock() {
  const { canInstall, installed, ios, hasNativePrompt, promptInstall } = useInstallPrompt();
  const { toast, node } = useToast();

  if (installed) {
    return (
      <Panel title="Application">
        <p className="text-[12.5px] text-good">Installée — tu utilises Le Cadran en plein écran depuis ton appareil.</p>
      </Panel>
    );
  }
  ...
  return (
    <Panel title="Application">
      ...
      <p className="mb-3 text-[12.5px] leading-relaxed text-ink-dim">
        {ios
          ? "Sur iPhone : Safari → icône Partager → « Sur l'écran d'accueil »."
          : "Installe Le Cadran sur ton écran d'accueil pour l'ouvrir en un geste, en plein écran."}
      </p>
      {hasNativePrompt && (
        <Button
          className="w-full"
